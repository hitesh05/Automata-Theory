...
int kya_hai_ye_yaar(int s1, int s2, vector<set<int>> &p, vector<pair<int, pair<char, int>>> &tf, set<int> &ls)
{
    for (auto l : ls)
    {
        int i1 = -1;
...
int main(int argc, char **argv)
{
    ...
    cout << blem.size() << " " << transition_matrix_count << " " << accept_state_count << '\n';
    set<int>::iterator it;
    for (int i = 0; i < blem.size(); i++)
    {
        for (it = letters.begin(); it != letters.end(); it++)
        {
            cout << i << " " << char(*it) << " " << grp_no[search(i, *it)] << "\n";
        }
    }
}
